...
def open_redis_client(
    ...
    if not redis_url:
        ...
        if not redis_url:
            client_loc_id = redis_prefix if redis_prefix else socket.gethostname().split('-')[0]
            client_locations = [location for location in redis_hosts if client_loc_id in location]
            if len(client_locations) != 1:
                raise RuntimeError("Failed to derive redis server url, please specify using the "
                                   "redis_url argument.")
            else:
    ...
    if redis_ssl:
        ...
        if not redis_pw:
            ...
            with open(redis_secret_file, 'r', encoding='utf-8') as password_file:
                redis_password = password_file.read().strip()
    else:
        redis_pw = None
